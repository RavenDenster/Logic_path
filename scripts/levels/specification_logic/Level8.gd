...
func _ready():
	level_data = preload("res://data/level_8_data.tres")
	super._ready()
	recount_gates()
	...
	for obj in movable_objects:
		if obj and is_instance_valid(obj):
			...
			if "ANDGate" in scene_file:
				...
				or_gate_count += 1
	print("Recounted gates - AND: ", and_gate_count, ", OR: ", or_gate_count)
...
func clear_level():
	...
	or_gate_count = 0
	update_gate_buttons_state()
	print("Level8 cleared - AND gate count reset to 0, OR gate count reset to 0")
...
func restore_level_state(state):
	...
	print("Level state restored successfully. AND gates: ", and_gate_count, ", OR gates: ", or_gate_count)
