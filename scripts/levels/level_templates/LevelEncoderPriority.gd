...
func _ready():
	...
	print("Priority Encoder level ready completed successfully")

func setup_encoder_priority_level():
	print("Setting up Priority Encoder level with 4 inputs and 2 outputs")
	...
	if input_block_i0_i1 and input_block_i2_i3:
		...
		print("Priority Encoder input blocks initialized")
	else:
		push_error("Input blocks not found in Priority Encoder level!")
	...
	if output_block_o0 and output_block_o1:
		...
		print("Priority Encoder output blocks initialized")
	else:
		push_error("Output blocks not found in Priority Encoder level!")

	test_results_panel = get_node_or_null("TestResultsPanelEncoderPriority")
	if test_results_panel:
		print("Priority Encoder test panel found")
...
func _on_test_pressed():
	print("=== Testing Priority Encoder level ===")
...
func propagate_signals():
	print("=== Starting signal propagation for Priority Encoder ===")
...
func clear_level():
	...
	print("Priority Encoder level cleared - kept Input/Output blocks, removed gates and wires")
...
func reset_all_port_sprites():
	...
	print("Priority Encoder level: Reset all port sprites")
...
# Пустые методы для удаления гейтов (будут переопределены в Level22.gd)
func remove_or_gate():
	pass
