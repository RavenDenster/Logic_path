...
func _ready():
	...
	custom_minimum_size = Vector2(0, 180)  # Увеличили высоту в 2 раза
...
func _update_panel_position():
	...
	custom_minimum_size = Vector2(window_size.x, 180)
	size = Vector2(window_size.x, 180)
	position = Vector2(0, window_size.y - 180)
	queue_redraw()
