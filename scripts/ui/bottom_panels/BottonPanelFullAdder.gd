...
func _ready():
	...
	custom_minimum_size = Vector2(0, 250)  # Увеличили высоту в 2 раза
	
	var viewport_size = get_viewport_rect().size
	size = Vector2(viewport_size.x, 250)
...
func _update_panel_position():
	...
	custom_minimum_size = Vector2(window_size.x, 250)
	size = Vector2(window_size.x, 250)
	position = Vector2(0, window_size.y - 250)
	queue_redraw()
